...
def test_bse_bhavcopy_parsing_and_canonical_deduplication(db):
    """Test BSE Bhavcopy ingestion and deduplication when NSE is already ingested."""
    service = BhavcopyService(db)
    trade_date = date(2026, 9, 1)

    # 1. First ingest NSE quotes (INFY)
    nse_csv = (
        "SYMBOL, SERIES, OPEN_PRICE, HIGH_PRICE, LOW_PRICE, CLOSE_PRICE, PREV_CLOSE, TTL_TRD_QNTY, DELIV_QTY, DELIV_PER, ISIN\n"
        "INFY, EQ, 1900.0, 1950.0, 1890.0, 1945.0, 1895.0, 1000000, 650000, 65.0, INE009A01021\n"
    )
    service.ingest_bhavcopy(Exchange.NSE, trade_date, nse_csv)

    # 2. Ingest BSE Bhavcopy with dual-listed INFY and BSE-exclusive stock
    bse_csv = (
    ...
    # Only 1 quote (BSEONLY) should be ingested because INFY is already present from NSE!
    count = service.ingest_bhavcopy(Exchange.BSE, trade_date, bse_csv)
    assert count == 1

    # Verify INFY retains NSE canonical quote with delivery data
    infy_row = db.conn.execute("""
        SELECT symbol, exchange, close_price, deliverable_volume, delivery_pct
        FROM eod_quotes WHERE isin = 'INE009A01021' AND trade_date = '2026-09-01';
    """).fetchone()
    assert infy_row == ("INFY", "NSE", 1945.0, 650000, 65.0)

    # Verify BSE-exclusive stock is ingested with BSE exchange tag
    bse_row = db.conn.execute("""
        SELECT symbol, exchange, close_price, total_volume
        FROM eod_quotes WHERE isin = 'INE888B01088' AND trade_date = '2026-09-01';
    """).fetchone()
    assert bse_row == ("BSEONLY", "BSE", 102.0, 50000)
